...
{    public partial class IdGenerator
    {
        public string CreateHex16Id([UniqueId] string id = null) => id ?? CreateHex16Id_String_id_Id;
        
        public string CreateHex32Id([UniqueId(UniqueIdFormat.Hex32)] string id = null) => id ?? CreateHex32Id_String_id_Id;
        
        public string CreateGuidId([UniqueId(UniqueIdFormat.Guid)] string id = null) => id ?? CreateGuidId_String_id_Id;
        
        public string CreateHex8Id([UniqueId(UniqueIdFormat.Hex8)] string id = null) => id ?? CreateHex8Id_String_id_Id;
        
        public string CreateHtmlId([UniqueId(UniqueIdFormat.HtmlId)] string id = null) => id ?? CreateHtmlId_String_id_Id;
        
        // Practical use cases with prefixes
        public string CreateUserId([UniqueId] string id = null) => ""usr_"" + (id ?? CreateUserId_String_id_Id);
        
        public string CreateSessionId([UniqueId(UniqueIdFormat.Hex8)] string id = null) => ""sess_"" + (id ?? CreateSessionId_String_id_Id);
        
        public string CreateTransactionId([UniqueId(UniqueIdFormat.Guid)] string id = null) => ""tx_"" + (id ?? CreateTransactionId_String_id_Id);
        
        // HTML elements with the HtmlId format
        public string CreateButtonId([UniqueId(UniqueIdFormat.HtmlId)] string id = null) => id ?? CreateButtonId_String_id_Id;
        
        public string CreateInputId([UniqueId(UniqueIdFormat.HtmlId)] string id = null) => id ?? CreateInputId_String_id_Id;
    }
}";
            ...
            AssertHasIdWithFormat(generatedCode, "CreateHex16Id_String_id_Id", @"^[a-f0-9]{16}$");
            AssertHasIdWithFormat(generatedCode, "CreateHex32Id_String_id_Id", @"^[a-f0-9]{32}$");
            AssertHasIdWithFormat(generatedCode, "CreateGuidId_String_id_Id", @"^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$");
            AssertHasIdWithFormat(generatedCode, "CreateHex8Id_String_id_Id", @"^[a-f0-9]{8}$");
            
            // HTML ID format should start with a letter and contain only valid characters
            var htmlIdMatch = Regex.Match(generatedCode, @"public const string CreateHtmlId_String_id_Id = ""([a-zA-Z0-9\-_:\.]+)"";");
...
namespace StaticTest
{
    ...
    {        public static string GetId([UniqueId] string id = null) => id ?? GetId_String_id_Id;
    }
    ...
    public partial class NonStaticClass
    {
        public string GetId([UniqueId] string id = null) => id ?? GetId_String_id_Id;
            ...
            Assert.Contains("public const string GetId_String_id_Id", staticClassSource.Source);
            ...
            Assert.Contains("public const string GetId_String_id_Id", nonStaticClassSource.Source);
        }
    ...
    namespace MiddleNamespace
    {
        ...
        public void IntegrationTest_HtmlId_ComplianceWithHtmlSpec()
        {
            ...
            var methodLines = string.Join("\n", Enumerable.Range(1, 50)
                .Select(i => $"public string GetHtmlId{i}([UniqueId(UniqueIdFormat.HtmlId)] string id = null) => id ?? GetHtmlId{i}_String_id_Id;"));
...
{{
    ...
    {{
        {methodLines}
            ...
            var generatedCode = generatedSource.Source;              // Extract all HTML IDs
            var matches = Regex.Matches(generatedCode, @"public const string GetHtmlId\d+_String_id_Id = ""([a-zA-Z0-9\-_]+)"";");
